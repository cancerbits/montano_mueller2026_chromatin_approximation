...
###by Luis F. Montano-Gutierrez. University of Edinburgh 2015-2024.
...
###################Roc curves
roc.curve<- function(matrix,step=0.01, true.value='T', false.value='F', color="black", line.width=1, plot.symbol=20, make.plot=T){
...
################################################################################
# a potentially more efficient way to calculate a roc matrix
################################################################################
get.roc.matrix=function(sp, signal.col=NULL, class.col="class"){
sp2=sp %>% dplyr::mutate(!!signal.col:= as.numeric(!!sym(signal.col))) %>% arrange(!!sym(signal.col)) %>% dplyr::mutate(pctall=1:nrow(!!sp)/nrow(!!sp)) %>% select(!!sym(class.col), !!sym(signal.col), pctall)
alltrues=sum(sp2[, class.col])
allfalses=sum(!sp2[, class.col])

rbind(c(TP=0, FP=0), 
lapply(1:nrow(sp2), function(x){
  sp2[1:x, ] %>% summarise( TPs=sum(!!sym(class.col), na.rm=T)/!!alltrues, FPs=sum(!(!!sym(class.col)), na.rm=T)/!!allfalses)
...
class2color.TF2<-function(x, colors=c("light blue", "orange"), nacolor="grey"){
 ...
 # plot.classified.proteins2(CCAN.rf, Nup.Ran.rf, name.vector=protein.groups[,8])
  ...
  trapezium.auc2 <- function(matrix) {
    # Sort the matrix by increasing FP
    #matrix <- matrix[order(matrix[,"FP"]),]
    
    # Calculate differences in FP and TP
    delta_fp <- diff(matrix[,"FPs"])

    
    auc=lapply(1:(nrow(matrix)-1), function(x){
    
    ((matrix[x, "TPs"]+matrix[x+1, "TPs"])*delta_fp[x])/2
    }) %>% Reduce('+', .)
  ...
  trapezium.auc<- function(roc.output.matrix, height=.001){
    ...
    #base1<-0
    # keep all the unique coodinates
    roc.output.matrix=rocmat[, c(2,3)] %>% unique %>% as.data.frame
    
    #reduce these coordinates further.
    #when there are only clinmbs (repeats on the FP axis) keep the highest tp value
    
    lapply(roc.output.matrix$FPs %>% unique, function(x){
